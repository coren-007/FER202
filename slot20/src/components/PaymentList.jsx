...
/**
 * Danh sách payments (đơn giản): tải dữ liệu, áp dụng filter/sort phía client.
 * Lưu ý: Sử dụng axios instance `api` (định nghĩa trong services/api) để fetch.
 * @param {{ filters?: {semester?: string, courseName?: string}, sortBy?: string }} props
 */
export default function PaymentList({ filters, sortBy }) {
  /**
   * Danh sách payments đã được lọc và sắp xếp.
   */
  ...
  /**
   * Tải toàn bộ danh sách payments.
   */
  useEffect(() => {
    load()
  }, [])
  ...
  /**
   * Áp dụng filter/sort khi props thay đổi.
   */
  useEffect(() => {
    applyFilterAndSort()
  }, [filters, sortBy])
  ...
  /**
   * Áp dụng bộ lọc và sắp xếp ở phía client.
   */
